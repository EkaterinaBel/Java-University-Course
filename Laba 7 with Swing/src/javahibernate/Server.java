...
public class Server extends Thread implements Serializable{
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (bol) {
                    ...
                    if (mapContainsChanges.size() != 0) {
                        ...
                        oos.writeObject(directory1.getPath());
                        physicalSynchronization();
                        oos.writeObject(nameFileServerUI);
                        oos.writeObject(sizeServerUI);
                        if (in.readObject().equals("size mapContainsChangesFromClient != 0")) {
                            ...
                            try {
                                while (k != mapContainsChangesFromClient.size()) {
                                    ...
                                    Path pathDestination = Paths.get(directory1 + File.separator + pair.getKey());
                                    Files.copy(pathSourse, pathDestination, REPLACE_EXISTING);

                                    File fileUI = new File(directory1 + File.separator + pair.getKey());
                                    sizeServerUI = sizeServerUI + fileUI.length();
                                    nameFileServerUI = fileUI.getPath();
                                    oos.writeObject(nameFileServerUI);
                                    oos.writeObject(sizeServerUI);
                                    k++;
                                }
                        ...
                        for (String o : mapContainsChanges.keySet()) {
                            if (mapContainsChanges.get(o).getName().indexOf(directory1.getPath()) == 0 &&
                                    !mapContainsChanges.get(o).getTypeFile()
    ...
    private void deleteAll(File file) {
        ...
        sizeServerUI = sizeServerUI + file.length();
        nameFileServerUI = file.getPath();
        file.delete();
    }
    ...
    private void physicalSynchronization() {
        ...
        for(String fileName: mapContainsChanges.keySet()) {
            ...
            if (mapContainsChanges.get(fileName).getFileOperations()) {
                ...
                nameFileServerUI = tmpDirectory.getPath();
                tmpDirectory.mkdir();
            }
        }
    }
